...
void StencilProbe(double *A0, double *Anext, int nx, int ny, int nz,
  ...
  omp_set_dynamic(0);  
  #pragma omp parallel for shared(A0, Anext,numBlocks_y,fac, nx, ny, nz, ty, timesteps, tz, tx) default(none) private(readQueuePlane0, readQueuePlane1, readQueuePlane2, writeQueuePlane, tempQueuePlane, blockMin_y, blockMax_y, writeBlockMin_y, writeBlockMax_y, writeBlockRealMin_y, writeBlockRealMax_y, readBlockUnitStride_y, writeBlockUnitStride_y, readOffset, writeOffset, i, j, k, s, t) firstprivate(queuePlanes, queuePlane0, queuePlane1, queuePlane2, queuePlanesIndices)
